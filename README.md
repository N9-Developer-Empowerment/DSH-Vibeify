...
- **A reviewed Vibe Social Desk.** The optional companion adds **Prepare social posts** to each finished card and a local **Social Desk** queue. It turns only that cleaned one-article snapshot into distinct channel drafts; prompts, Chat history, reasoning, other cards, account data and credentials do not cross the boundary. X, Bluesky, Threads, Facebook Pages and professional Instagram can use their official APIs after one explicit **Approve and schedule** action for the exact final copy and time. Reddit, Discord, YouTube Community, Facebook personal profiles and any unsupported or unconfigured route remain **Ready to post**, with copy/open helpers rather than disguised browser automation. Missed and uncertain posts return to review instead of flooding out after restart. See [Vibe Social Desk](docs/SOCIAL_DESK.md).
