...
| **Vibe** | Presents completed answers from every thread as one newest-first edition. A hidden local reserve may be prepared while Vibe is in recent use; only Pull to update, Update, or a completed Chat answer changes the visible magazine. It is not a separate public site. |
...
- **One magazine across every real Chat thread.** Vibe reads local DSH history without reopening or resuming a thread and accepts only turns durably marked completed. Internal subagents, the dedicated foreground update and the hidden background editor are never mistaken for reader conversations. Transport envelopes are removed during cache migration; card titles are not repeated in their bodies, and Markdown tables remain structured. Raw prompts, worker reports, candidate lists, attachments, reasoning, tool activity, approvals, session identifiers, aborted work and incomplete progress are never copied.
