...
An explicit update can then extend that reserve with remote public imagery. The generated envelope places one allow-listed direct image URL and its separate source-page link at the start of a complete card. `feed.js` validates the image host before rendering; `shell.jsx` suppresses referrer data, falls back to the local topic asset on load failure, and passes the 24 most recent accepted image URLs into the next update contract for de-duplication. The complete card remains subject to `content-store.js`'s existing 30-day and 160-item bounds, so the rolling catalogue naturally sheds stale material without a background refresher or a second storage system.
