...
The Cloudflare-compatible service in `services/vibe-share/` owns the second confirmation, bounded abuse protection, D1 storage, public rendering, social metadata, retention, and capability-based removal token. It stores no account and DSH holds no publishing secret. The default managed protection stores only a salted one-day request fingerprint and counters; Turnstile remains an optional replacement. Production publishing fails closed when storage or protection is absent. The public renderer escapes raw HTML, permits only HTTPS article links, keeps image credits attached, and applies a restrictive content-security policy. The `share` subdomain uses the managed host's external-DNS CNAME, so the existing apex site, nameservers, and mail records do not move. This service can be deployed or removed independently from DSH; the local magazine remains functional when it is unavailable.
