...
- `plugins/dsh-social-desk/social-desk-service.js` owns the local draft/approval/schedule/status state machine, bounded retry policy, missed-schedule recovery and exact-copy approval receipt. `official-connectors.js` owns official X, Bluesky, Threads, Facebook Page and professional Instagram requests; it resolves credentials per request and returns only a remote id or public URL. `social-rpc.js` exposes the service only to the loopback browser. Manual community channels have no connector.
...
- Social access tokens and app passwords are credential references, not queue or repository values. **Settings → Vibe Social Desk** owns local channel enablement and public account fields. `dsh-social-desk` resolves the selected credential only for an approved due item and never returns it through RPC.
