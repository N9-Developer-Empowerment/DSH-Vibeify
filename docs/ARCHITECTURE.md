...
The Cloudflare Pages Function in `services/vibe-share/` owns the second confirmation, bot check, D1 storage, public rendering, social metadata, retention, and capability-based removal token. It stores no account and DSH holds no publishing secret. Production publishing fails closed when storage or the human-check secret is absent. The public renderer escapes raw HTML, permits only HTTPS article links, keeps image credits attached, and applies a restrictive content-security policy. The `share` subdomain can use an external-DNS CNAME to the Pages project, so the existing apex site, nameservers, and mail records do not move. This service can be deployed or removed independently from DSH; the local magazine remains functional when it is unavailable.
