...
- `plugins/dsh-social-desk/social-desk-service.js` owns the local draft/approval/schedule/status state machine, missed-schedule recovery and exact-copy approval receipt. Every channel defaults to a credential-free composer hand-off: when due, the local item becomes Ready to post and makes no external request. `official-connectors.js` owns the explicitly enabled X, Bluesky, Threads, Facebook Page and professional Instagram alternative; it resolves credentials per approved request and returns only a remote id or public URL. `social-rpc.js` exposes the service only to the loopback browser.
- `client-src/experience/social-desk-client.js` owns the optional loopback client and sends only a cleaned share snapshot. `social-desk-panel.jsx` owns the nontechnical queue, final-copy editor, schedule control and Ready-to-post helpers. Absence of `dsh-social-desk` is a normal supported state: all social controls are hidden and Vibe/share continue unchanged.
...
- Social access tokens and app passwords are optional credential references, not queue or repository values. **Settings → Vibe Social Desk** owns advanced official-channel enablement and public account fields. With all switches off, every channel stays on the no-API composer route. `dsh-social-desk` resolves a selected credential only for an approved due official item and never returns it through RPC.
