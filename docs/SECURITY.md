...
- Progressive editor chunks are local presentation events from one explicitly requested Codex-led magazine update or from a user-explicit show/find/browse/recommend request for public reader-facing content. The dedicated session's loopback mux deltas are accumulated only until a complete active-run envelope closes; final history is the fallback. Only complete closed envelopes accepted by the Codex lead are eligible for the persistent magazine. Ordinary progress, private source material, incomplete envelopes, subagent sessions and raw worker output are not eligible.
...
- A user-requested magazine Update may add newly verified public imagery to the rolling catalogue. The browser renders direct HTTPS images only from the reviewed host allow-list, removes unrecognised query parameters, requires and keeps the human-readable source link visible, strips the lead image from displayed article Markdown, suppresses the HTTP referrer, and falls back to a bundled local asset on failure. The remote host still receives the reader's IP address when its cached image is displayed, including on a later open or lazy-loaded scroll. Arbitrary image hosts and recently used URLs are excluded; cached image metadata expires with the same 30-day/160-item content bound. Opening or scrolling never renews the catalogue or starts model work.
