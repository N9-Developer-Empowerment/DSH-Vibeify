...
function serviceFixture({ now = Date.UTC(2026, 7, 31, 10, 0, 0), publish } = {}) {
...
test("preparing one cleaned Vibe creates direct and ready-to-post drafts without private material", async () => {
  const { service } = serviceFixture();
  const result = await service.prepare({ snapshot, channels: ["x", "reddit"] });

  assert.equal(result.items.length, 2);
  assert.equal(result.items.find(({ channel }) => channel === "x").mode, "official-api");
  assert.equal(result.items.find(({ channel }) => channel === "reddit").mode, "ready-to-post");
...
test("one approve-and-schedule action freezes exact copy and authorizes one later API publish", async () => {
  const published = [];
  const { service } = serviceFixture({ publish: async (item) => {
    published.push(item);
...
test("manual community channels become Ready to post and never call a publisher", async () => {
  const { service } = serviceFixture({ publish: async () => { throw new Error("must not publish"); } });
  const prepared = await service.prepare({ snapshot, channels: ["reddit", "discord", "youtube-community"] });

  for (const draft of prepared.items) {
    const ready = await service.approveAndSchedule({ id: draft.id, revision: draft.revision, text: draft.text, scheduledAt: null });
    assert.equal(ready.status, "ready-to-post");
  }
  assert.equal((await service.tick(Date.UTC(2026, 7, 31, 11, 0, 0))).attempted, 0);
});
...
test("a connector failure records a bounded retry without leaking remote response bodies", async () => {
  const { service } = serviceFixture({ publish: async () => {
