...
- An explicit update must spend its local visual short and questionnaire immediately, then publish each complete closed generated chunk from the live DSH stream as soon as the lead verifies it. Never wait for the parent final answer before showing ready chunks, and never project a subagent report, candidate list, research memo, or dedicated-update summary as a Vibe article.
