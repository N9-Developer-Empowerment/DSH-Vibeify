...
export function articleMarkup(snapshot, { includeTitle = true } = {}) {
  ...
  return `<article class="article">${visualHtml(lead)}<div class="copy"><span class="kind">${escapeHtml(snapshot.kind)}</span>${includeTitle ? `<h1>${escapeHtml(snapshot.title)}</h1>` : ""}<div class="body">${markdownToHtml(snapshot.markdown)}</div>${mediaHtml(snapshot.media)}${gallery}${source}</div></article>`;
}
